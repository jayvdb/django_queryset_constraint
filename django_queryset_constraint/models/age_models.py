...
class Disallow1CC(AgeModel):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(name="CC: Disallow age=1", check=~Q(age=1))
        ]
...
class Disallow1QC(AgeModel):
    ...
    class Meta:
        constraints = [
            QuerysetConstraint(
                name="QC: Disallow age=1", queryset=M().objects.filter(age=1)
            )
        ]
    ...
    class Meta:
        constraints = [
            QuerysetConstraint(
                ...
                queryset=M()
                .objects.annotate(new_age=RawSQL("NEW.age", ()))
                .filter(new_age=1),
            )
        ]
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                name="CC: Disallow age in list", check=~Q(age__in=[1, 2])
            )
        ]
...
class Disallow12MultiFilterCC(AgeModel):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                name="CC: Disallow age Multi1 filter", check=~Q(age=1)
            ),
            models.CheckConstraint(
                name="CC: Disallow age Multi2 filter", check=~Q(age=2)
            ),
        ]
...
class Disallow12MultiFilterMixed(AgeModel):
    class Meta:
        constraints = [
            models.CheckConstraint(
                name="CC: Disallow age MultiMixed filter", check=~Q(age=1)
            ),
            QuerysetConstraint(
...
class AllowOnly0CC(AgeModel):
    class Meta:
        constraints = [
            models.CheckConstraint(name="CC: Allow only 0", check=Q(age=0))
        ]
...
class AllowOnly0QC(AgeModel):
    class Meta:
        constraints = [
            QuerysetConstraint(
                name="QC: Allow only 0", queryset=M().objects.exclude(age=0)
            )
        ]
...
class AllowOnly1ObjectQC(AgeModel):
    class Meta:
        constraints = [
            ...
            QuerysetConstraint(
                name="QC: Allow only 1 object", queryset=M().objects.all()[1:]
            )
        ]
    ...
    class Meta:
        constraints = [
            QuerysetConstraint(
                ...
                queryset=M()
                .objects.annotate(collision=Exists(M().objects.filter(age=1)))
...
def generate_subquery(layers):
    ...
    inner_queryset = M().objects.filter(age=1)
    return_queryset = inner_queryset
    for _ in range(layers):
        return_queryset = (
            M()
            .objects.annotate(collision=Exists(return_queryset))
            .filter(collision=True)
        )
